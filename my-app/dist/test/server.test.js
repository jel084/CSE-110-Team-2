...
describe('/markFound tests', () => {
    test('PUT /markFound should mark an item as found', () => __awaiter(void 0, void 0, void 0, function* () {
        // Create a lobby with items
        yield db.run(`
      INSERT INTO lobbies (lobbyName, host, players, scavengerItems, points, pin, gameTime, status) VALUES
      ('New Lobby 1', 'Host 1', '["Player 1"]', '[{"id":1,"name":"Triton Statue","points":10,"found":false},{"id":2,"name":"Sun God","points":10,"found":false}]',
    ...
    `);
        yield db.run(`
      INSERT INTO player_items VALUES ('Player 1', 1, 1, 0, ''), ('Player 1', 1, 2, 0, '')
    `);
        // Perform the PUT request to the appropriate /markFound endpoint
        const res = yield axios_1.default.put(`http://localhost:${PORT}/api/lobbies/1/players/Player 1/items/markFound`, {
            items: [1, 2]
        });
        expect(res.status).toBe(200);
        expect(res.data).toMatchObject({ message: 'Items marked as found successfully' });
        // Ensure the player_items table is updated
        const player_items = yield db.all(`SELECT * FROM player_items WHERE player_id = 'Player 1' AND lobby_id = 1`);
        expect(player_items).toHaveLength(2);
        expect(player_items[0]).toMatchObject({
            player_id: 'Player 1',
            lobby_id: 1,
    ...
    test('PUT /markFound with invalid lobby id should return error', () => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        try {
            yield axios_1.default.put(`http://localhost:${PORT}/api/lobbies/1/players/Player 1/items/markFound`, {
                items: [1, 2]
            });
        }
        catch (error) {
            if (axios_1.default.isAxiosError(error)) {
                expect((_a = error.response) === null || _a === void 0 ? void 0 : _a.status).toBe(500);
                expect((_b = error.response) === null || _b === void 0 ? void 0 : _b.data).toMatchObject({
                    error: 'Failed to mark items as found'
                });
            }
            else {
                throw error;
...
describe('/saveScores tests', () => {
    test('POST /saveScores should save game results to the results table', () => __awaiter(void 0, void 0, void 0, function* () {
        // Perform the POST request to the appropriate /saveScores endpoint
        const res = yield axios_1.default.post(`http://localhost:${PORT}/api/lobbies/1/saveScores`, {
            players: [{ id: "Player 1", points: 10 }, { id: "Player 2", points: 0 }]
        });
        expect(res.status).toBe(200);
        expect(res.data.message).toBe('Scores saved successfully');
        // Ensure the results table is updated
        const results = yield db.all('SELECT * FROM results');
        ...
        yield db.run('DROP TABLE IF EXISTS results;');
    }));
    test('POST /saveScores with no results table should return error', () => __awaiter(void 0, void 0, void 0, function* () {
        var _a, _b;
        yield db.run(`DROP TABLE results`);
        try {
            yield axios_1.default.post(`http://localhost:${PORT}/api/lobbies/1/saveScores`, {
                players: [{ id: "Player 1", points: 10 }, { id: "Player 2", points: 0 }]
            });
        }
        catch (error) {
