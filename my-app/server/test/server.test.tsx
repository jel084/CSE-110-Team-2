...
describe('/markFound tests', () => {
  test('PUT /markFound should mark an item as found', async () => {
    // Create a lobby with items
    await db.run(`
      INSERT INTO lobbies (lobbyName, host, players, scavengerItems, points, pin, gameTime, status) VALUES
      ('New Lobby 1', 'Host 1', '["Player 1"]', '[{"id":1,"name":"Triton Statue","points":10,"found":false},{"id":2,"name":"Sun God","points":10,"found":false}]',
    ...
    const res = await axios.put(`http://localhost:${PORT}/api/lobbies/1/players/Player 1/items/markFound`, {
      items: [1, 2]
    });
    ...
    // Ensure the player_items table is updated
    const player_items = await db.all(`SELECT * FROM player_items WHERE player_id = 'Player 1' AND lobby_id = 1`);
    expect(player_items).toHaveLength(2);
    expect(player_items[0]).toMatchObject({
      player_id: 'Player 1',
      lobby_id: 1,
  ...
  test('PUT /markFound with invalid lobby id should return error', async () => {
    try {
      await axios.put(`http://localhost:${PORT}/api/lobbies/1/players/Player 1/items/markFound`, {
        items: [1, 2]
      });
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
...
describe('/saveScores tests', () => {
  test('POST /saveScores should save game results to the results table', async () => {
    // Perform the POST request to the appropriate /saveScores endpoint
    const res = await axios.post(`http://localhost:${PORT}/api/lobbies/1/saveScores`, {
    ...
    await db.run('DROP TABLE IF EXISTS results;');
  });

  test('POST /saveScores with no results table should return error', async () => {
    await db.run(`DROP TABLE results`);

    try {
      await axios.post(`http://localhost:${PORT}/api/lobbies/1/saveScores`, {
        players: [{id: "Player 1", points: 10}, {id: "Player 2", points: 0}]
      });
    } catch (error: unknown) {
      if (axios.isAxiosError(error)) {
